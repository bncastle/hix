...
class Util {
	...
	public static function SetExt(filename:String, ext:String) : String{
		var path = new Path(filename);
		path.ext = ext;
	...
	public static function PopNextArg(args:Array<String>):String {
	...
	// returns null, otherwise
	public static function PopFirstNonFilename(args:Array<String>):String {
		var i = args.length - 1;
		while (i >= 0) {
			var p = new Path(args[i]);
			if (p.ext == null) {
				return StringTools.trim(args.splice(i, 1)[0]);
			}
			i--;
		}
		return null;
